...
def _update(sampler, key, model, x, K, N, tau):
...
def main(_):
    ...
    model_def = VAE([512, 256, FLAGS.K*FLAGS.N], [FLAGS.K*FLAGS.N, 256, 512, 784], FLAGS.N, FLAGS.K)
    model = Model.create(model_def, inputs=[key, rng, np_x, sampler, tau], tx=optax.adam(learning_rate=FLAGS.lr))
    ...
    for i in range(1,FLAGS.epochs+1):
        for step, (np_x, _) in enumerate(data.take(6000), 1):
            rng, new_model, info = _update(sampler, rng, model, jnp.array(np_x), FLAGS.N, FLAGS.K, tau)
            model = new_model
            if step % 1000 == 0:
